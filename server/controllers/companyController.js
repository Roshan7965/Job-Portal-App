...
export const postJob = async (req, res) => {
  ...
  try {
    ...
    if (existingJob) {
      return res
        .status(409)
        .json({ success: false, message: "Job already posted." });
    }
...
export const changeJobApplicationStatus = async (req, res) => {
  try {
    ...
    if(!status || !id){
        return res.json({success:false,message:"status and id not present"})
    }
...
export const changeVisibility = async (req, res) => {
  try {
    ...
    if (companyId.toString() === job.companyId.toString()) {
      ...
      return res
        .status(200)
        .json({ success: true, job, message: "change the status" });
    } else {
      return res
        .status(403)
